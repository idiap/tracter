...
Tracter::BSAPIFastVTLN::BSAPIFastVTLN(Component<float>* iInput, const char* iObjectName)
{
    ...
    Connect(mInput,  MaxBufferedFrames);
    ...
    mFrame.size=1;   // just Warp factors
 ...
 */
bool Tracter::BSAPIFastVTLN::UnaryFetch(IndexType iIndex, float* oData)
{
    assert(iIndex >= 0);
    CacheArea inputArea;
    int extend,shift;
